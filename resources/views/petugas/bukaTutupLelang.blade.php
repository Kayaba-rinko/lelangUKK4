    ...
    <div class="main-content">
        <div class="header">
            ...
            <div class="header-right">
                ...
                <form action="{{ route('petugas.bukaTutup.cari') }}" class="search-box no-bg">
                    <input style="font-size:16px" type="text" name="cari" placeholder="Cari Lelang..."value="{{ request('cari') }}">
                    <button type="submit" class="btn-search">🔍</button>
                </form>
                <a href="{{ route('petugas.bukaTutup.create') }}" class="btn-primary tambah-btn">Tambah Lelang</a>
            </div>
            </div>
        </div>
                ...
                <tbody>
                    @foreach ($lelang as $lelangu)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('petugas.bukaTutup.edit', $lelangu->id_lelang) }}"class="btn-primary">Edit</a>
                                <form action="{{ route('petugas.bukaTutup.destroy', $lelangu->id_lelang) }}"method="POST"style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                                    <button onclick="return confirm('Yakin ingin menghapus lelang ini?')"class="btn-primary">Hapus</button>
                                </form>
